...
function App() {
    ...
    const fetchCurrentUser = async () => {
        const res = await fetch('https://comments-db-luckychimp.herokuapp.com/currentUser');
    ...
    const fetchComments = async () => {
        const res = await fetch('https://comments-db-luckychimp.herokuapp.com/comments');
        const data = await res.json();
    ...
    const updateScore = async (id, change) => {
        ...
        const res = await fetch(`https://comments-db-luckychimp.herokuapp.com/comments/${updComment.id}`, {
            method: 'PUT',
            headers: {
    ...
    const deleteComment = async () => {
        ...
        if (isReply(comment)) {
            ...
            parentComment.replies.splice(parentComment.replies.findIndex(reply => reply === comment), 1);

            await fetch(`https://comments-db-luckychimp.herokuapp.com/comments/${parentComment.id}`, {
                method: 'PUT',
                headers: {
                    'Content-type': 'application/json'
                },
            ...
            setComments(
                comments.map(comment => (comment.id === parentComment.id) ? parentComment : comment)
            );
        } else {
            await fetch(`https://comments-db-luckychimp.herokuapp.com/comments/${deleteCommentID}`, {
                method: 'DELETE'
            });
    ...
    const updateCommentText = async (id, txt) => {
        ...
        const comment = await getCommentByID(id);
        const updCommentOrReply = isReply(comment) ? { ...await getCommentByID(id), content: textContent, replyingTo: replyingTo} : { ...await getCommentByID(id), content: textContent};
        const updComment = await getParentWithComment(updCommentOrReply);
    ...
    const createReply = async (replyingTo, text) => {
        ...
        const res = await fetch(`https://comments-db-luckychimp.herokuapp.com/comments/${comment.id}`, {
            method: 'PUT',
            headers: {
                'Content-type': 'application/json'
            },
